...
const Table = () => {
    ...
    return (
        <div className='flex flex-col mt-9 border border-gray-100 rounded'>
            {data ? <table className='w-full table-auto'>
                ...
                <tbody>
                    {
                        data.map((row, i) => {
                            return (<tr key={i} className='text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0'>
                                {/* <td className='py-4 '></td> */}
                                <td className='py-4 flex items-center uppercase'><button className='outline-0 border-0 bg-none cursor-pointer'>
                                    ...
                                    <img src={row.image} alt="coin logo" className='w-[1.2rem] h-[1.2rem] mx-1.5' />
                                    {row.symbol}
                                </td>
                                <td className='py-4 '>{row.name}</td>
                                <td className='py-4 '>{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(row.current_price)}</td>
                                <td className='py-4 '>{row.total_volume}</td>
                                <td className='py-4 '>{row.market_cap_change_percentage_24h}%</td>
                                <td className={`py-4 ${row.price_change_percentage_1h_in_currency > 0 ? 'text-green' : 'text-rose-500'}`}>{Number(row.price_change_percentage_1h_in_currency).toFixed(3)}</td>
                                <td className={`py-4 ${row.price_change_percentage_24h_in_currency > 0 ? 'text-green' : 'text-rose-500'}`}>{Number(row.price_change_percentage_24h_in_currency).toFixed(3)}</td>
                                <td className={`py-4 ${row.price_change_percentage_7d_in_currency > 0 ? 'text-green' : 'text-rose-500'}`}>{Number(row.price_change_percentage_7d_in_currency).toFixed(3)}</td>
                            </tr>)
                        })
                    }
